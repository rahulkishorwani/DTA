...
abstract class Transition extends AutomatonComponent
    ...
    private String[] clocksToReset = null;
	...
	public String[] getClocksToReset() {
		return clocksToReset;
	}

	public void setClocksToReset(String[] clocksToReset) {
    ...
    public Transition(Automaton automaton, State src, State dst) {
        super(automaton);
        this.src = src;
        this.dst = dst;
    ...
    public boolean transitsOn(String what) {
        //return transits.indexOf(what) >= 0;
        if (alphabet.equals(what)) {
			return true;
		}
        return false;
    }
    public boolean canTransit() {
        //return !transits.equals("");
    	if(alphabet!=null)
    		return true;
    	return false;
    }
    ...
    public String determineLabelText() {
    	...
    	if(clocksToReset != null && clocksToReset.length >0){
    		ret.append(", ");
    		ret.append(Arrays.toString(clocksToReset));
    	}
    	return ret.toString();
    ...
    public void showMenu(int x, int y) {
    	CustomDialog customDialog = new CustomDialog(Main.getMainFrame(),this);
        customDialog.pack();
    ...
    public void print(GroupedWriter fout) {
        super.print(fout);
        fout.print("alphabet "); fout.printlnGroup(alphabet);
        if(guard != null && guard.length() > 0){
            fout.print("guard ");
            fout.printlnGroup(guard);
        }
        if(clocksToReset != null && clocksToReset.length > 0){
            fout.print("clocksToReset ");
            fout.printlnGroup(Arrays.toString(clocksToReset));
    ...
    public boolean setKey(String key, GroupedReader fin) throws IOException {
        ...
        else if(key.equals("clocksToReset")){
        	String clocksStr = fin.readGroup();
        	clocksStr = clocksStr.substring(1, clocksStr.length()-1);
        	clocksToReset = clocksStr.split(", ");
        	return true;
        }
	...
	public String toString() {
		return "Transition [src=" + src + ", dst=" + dst + ", alphabet=" + alphabet
				+ ", clocksToReset=" + Arrays.toString(clocksToReset)
				+ ", guard=" + guard+"]\n";
	}
}
